...
select * from car_details, car_category where category = category_name and available = true order by cost_per_day where category_name = "economy";
...
select Make,Model,Model_Year from car_details join booking_details where 
Registration_No = Car_Reg_No
EXCEPT
select Make,Model_name,Model_Year from car_detail where Model_Year <= 2012;
